...
上流の `last30days` はエージェント向けの skill です。Claude Code、Codex、Cursor などのホストにインストールし、そこでスラッシュコマンドを実行します。このフォークは同じエンジンを**セルフホストのリサーチコンソール**として動かします - ページを開き、トピックを入力し、エンジンの 26 あるソースのうちどれを実行するかを選び、日付付きの HTML レポートをその根拠データとともに読みます。実行結果はトピックごとにまとまり、ヘッダーには対象期間が表示され、各カードはそのトピックのスナップショットを日付付きのピルとして並べ、選んで開けます。ソースバーは種類ごとにグループ化されています。

- **新しいのはラッパーだけ** - `Dockerfile`、`docker-compose.yaml`、`entrypoint.sh`、`scripts/serve.py`（標準ライブラリのみの HTTP サーバーとリサーチコンソール: 期間を示すヘッダー、選択できるスナップショットのピルを持つトピックカード、グループ化されたソースの切り替え、レポートと根拠のページ、スナップショット単位とトピック単位の削除）に加えて `.env.example`、`README.docker.md`、`FORK.md`、`MEMORY.md`。
